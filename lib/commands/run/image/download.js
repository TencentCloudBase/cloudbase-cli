...
let DownLoadImage = class DownLoadImage extends common_1.Command {
    ...
    execute(envId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let imageUrl = yield utils_1.pagingSelectPromp('select', run_1.listImage, { envId, serviceName, limit: 0, offset: 0 }, '请选择下载的镜像', _ => true, item => item.ImageUrl);
            ...
            loading.start('正在拉取中');
            let sh = new Promise((resolve, reject) => child_process_1.exec(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker pull ${imageUrl}`, (err, stdout) => err ? reject({ code: -1, info: err }) : resolve({ code: 0, info: stdout })).stdout.pipe(process.stdout));
            let res = yield sh;
            if (res.code === -1)
