...
let UploadImage = class UploadImage extends common_1.Command {
    ...
    execute(envId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let stderr;
            if (stderr = (yield util.promisify(child_process_1.exec)(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker tag ${imageId} ccr.ccs.tencentyun.com/${imageRepo}:${imageTag}`)).stderr)
                throw new error_1.CloudBaseError(stderr);
            let sh = new Promise((resolve, reject) => child_process_1.exec(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker push ccr.ccs.tencentyun.com/${imageRepo}:${imageTag}`, (stderr, stdout) => stderr ? reject({ code: -1, info: stderr }) : resolve({ code: 0, info: stdout })).stdout.pipe(process.stdout));
            try {
                let res = yield sh;
                if (res.code === -1)
