...
let UploadImage = class UploadImage extends common_1.Command {
    ...
    execute(envId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { imageId } = yield enquirer_1.prompt({
                type: 'input',
                message: '请输入上传镜像id（ImageId）',
                name: 'imageId'
            });
            const { tag } = yield enquirer_1.prompt({
                type: 'input',
                message: '请输入镜像tag',
                name: 'tag'
            });
            loading.start('登陆中');
            let { stdout, stderr } = yield util.promisify(child_process_1.exec)(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker login --username=${uin} ccr.ccs.tencentyun.com -p ${pw}`);
            if (stdout.search('Login Succeeded') === -1)
                throw new error_1.CloudBaseError(stderr);
            loading.succeed('登录成功');
            if (stderr = (yield util.promisify(child_process_1.exec)(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker tag ${imageId} ccr.ccs.tencentyun.com/tcb-100017846734-ljoc/hello-tcb-9glo12vd5bd3bd0d_dartapp:${tag}`)).stderr)
                throw new error_1.CloudBaseError(stdout);
            loading.start('正在上传中');
            let sh = new Promise((resolve, reject) => child_process_1.exec(`${process.platform.search('win') === -1 ? 'sudo ' : ''}docker push ccr.ccs.tencentyun.com/${imageRepo}:${tag}`, (err, stdout) => err ? reject({ code: -1, info: err }) : resolve({ code: 0, info: stdout })).stdout.pipe(process.stdout));
            let res = yield sh;
            if (res.code === -1)
