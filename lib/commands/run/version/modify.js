...
const modifyByFlow = (envId, serviceName) => __awaiter(void 0, void 0, void 0, function* () {
...
const modifyByURL = (envId, serviceName) => __awaiter(void 0, void 0, void 0, function* () {
    const versionFlowItems = [];
    const loading = utils_1.loadingFactory();
    let Priority = 1;
    while (true) {
        let versionName = yield utils_1.pagingSelectPromp('select', run_1.listVersion, { envId, serverName: serviceName }, `请选择优先级${Priority}的版本`, (item) => item.Status === 'normal', (item) => `${item.VersionName}|${item.Remark}`);
        versionName = versionName.split('|')[0];
        const { key } = yield enquirer_1.prompt({
            type: 'input',
            name: 'key',
            message: `请输入优先级${Priority}条件的URL-key`
        });
        const { value } = yield enquirer_1.prompt({
            type: 'input',
            name: 'value',
    ...
    let versionName = yield utils_1.pagingSelectPromp('select', run_1.listVersion, { envId, serverName: serviceName }, '请选择默认的版本', (item) => item.Status === 'normal', (item) => `${item.VersionName}|${item.Remark}`);
    versionName = versionName.split('|')[0];
