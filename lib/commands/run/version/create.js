...
let CreateVersion = class CreateVersion extends common_1.Command {
    get options() {
        return Object.assign(Object.assign({}, common_2.versionCommonOptions('create')), { options: [
                {
                    flags: '-e, --envId <envId>',
                    desc: '环境 Id'
                },
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (uploadType === '本地代码') {
                ...
                if (fs_1.statSync(path).isDirectory()) {
                    yield run_1.packDir(path, './code.zip');
                    path = './code.zip';
                }
            }
            ...
            flowRatio = (yield enquirer_1.prompt({
                type: 'select',
                name: 'flow',
                message: '请选择流量策略',
                choices: ['保持流量为0， 稍后设置', '直接开启100%流量']
            })).flow === '直接开启100%流量' ? 100 : 0;
            versionRemark = (yield enquirer_1.prompt({
                type: 'input',
                name: 'versionRemark',
                message: '请输入备注',
            })).versionRemark;
            ...
            try {
                ...
                if (uploadType === '本地代码') {
                    ...
                    fs_1.unlinkSync('./code.zip');
                    loading.succeed('成功删除本地压缩包');
                    let response = yield run_1.createVersion({
                        envId,
